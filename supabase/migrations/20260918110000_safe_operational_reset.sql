-- Safe operational reset.
-- Preserves staff accounts and all queue/appointment history. Refuses to run
-- while live queue or upcoming appointment work exists.
...
begin
    ...
    if p_scope in ('global', 'announcement') then
        update public.shop_settings set shop_announcement = '', shop_announcement_html = '', shop_announcement_enabled = true;
    end if;
    if p_scope in ('global', 'closedDates') then
        update public.shop_settings set closed_dates = '[]'::jsonb;
    end if;
    if p_scope in ('global', 'hours') then
        update public.shop_settings set weekly_op_hours = v_weekly, booking_advance_days = 30;
    end if;
    if p_scope in ('global', 'services') then
        ...
        update public.staff set capability_service_ids = null, specialty_service_ids = null, service_durations = null;
    end if;
    if p_scope in ('global', 'seats') then
        ...
        update public.shop_settings set seat_count = 3;
    end if;
