...
for (const m of clean.matchAll(/public\.(\w+)/g)) {
    ...
    if (/^(is_active_staff|is_admin|bump_row_version|handle_new_staff_user|next_ticket_number|cancel_own_ticket|call_next_customer|complete_service|barber_performance|_appointment_slot_capacity_ok|_appointment_hours_ok|book_appointment|cancel_own_appointment|reschedule_own_appointment|convert_walkin_to_appointment|checkin_appointment|approve_fast_pass|revoke_fast_pass|list_today_queues_full|list_active_appointments|admin_cancel_record|reset_operational_state|_current_business_date|enforce_staff_self_update_scope|recompute_walkin_price_from_services|admin_remove_staff|sync_queue_barber_name|_business_day_start|list_today_queues|_notify_queues_change|_notify_appointments_change)$/.test(n)) continue;
