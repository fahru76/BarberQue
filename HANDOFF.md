...
## Done — the remaining two `/frontend-component-build` findings: ARIA tabs + dialog-mechanics consolidation (2026-09-11)

- Fahru said "complete the remaining" after the priority-recommendations
  round above shipped -- this entry covers findings #4 and #5 from
  `component-audit-2026-09-11.md`, both previously left as documented,
  intentional debt.
- **4. Mode toggle and weekly-hours day selector are now real ARIA
  tabs.** `#btnModeWalkin`/`#btnModeBooking` and the 7 `opDay{n}`
  weekly-hours buttons swapped an entire content panel via
  `aria-pressed` toggle buttons -- functionally usable, but a screen
  reader had no way to announce them as a tab group. Both are now
  `role="tablist"`/`role="tab"`/`aria-selected`, with the panel each
  group controls marked `role="tabpanel"`:
  `walkin-section`/`booking-section` for the mode toggle (each
  `aria-labelledby` its own tab), and a new shared
  `#operationalHoursPanel` wrapper for the weekly-hours fields (one
  panel, not seven -- its `aria-labelledby` is repointed to whichever
  day is currently selected by `renderOperationalDayTabs()`, since the
  7 day buttons all drive the same editable form rather than each
  having separate content). New `initTablistArrowNav()` gives both
  tablists Left/Right/Up/Down/Home/End keyboard navigation via a
  roving `tabindex` (only the selected tab is in the page's normal Tab
  order). **Deliberately manual activation**, not the APG's automatic-
  activation variant: arrow keys move focus only, Enter/Space/click
  still activates via each button's existing `onclick`. This was a
  specific, considered choice -- `selectOperationalDay()` can pop an
  unsaved-changes confirm dialog, so auto-selecting on every arrow
  keypress while a staff member browses through the 7 days would fire
  that dialog repeatedly. `setCustomerMode()` and
  `renderOperationalDayTabs()` were updated to set `aria-selected` +
  roving `tabindex` instead of `aria-pressed` (their non-ARIA behaviour
  -- `className`/`classList` toggling, panel `display`, label text --
  is untouched). The main top-nav (`navCustomerBtn` etc.) and the
  announcement rich-text toolbar buttons still use `aria-pressed`
  deliberately -- they're real independent toggles/views, not tabs
  sharing one panel, so they were correctly out of this finding's
  scope and left alone.
- **5. Dialog open/close mechanics consolidated.** The
  `showModal()`/`setAttribute('open')` and
  `close()`/`removeAttribute('open')` fallback pair (for browsers
  without native `<dialog>` support) was copy-pasted at 8 separate
  call sites across `showAlertDialog()`, `showConfirmDialog()`, and
  all 4 hand-built dialogs the audit flagged as a parallel pattern
  (`adminCancelDialog`, plus `staffLoginDialog`/`setPasswordDialog`,
  which turned out to have the exact same duplication even though the
  audit only named `adminCancelDialog` and `stylePhotoDialog`
  explicitly). New `openAppDialog(dialog)`/`closeAppDialog(dialog)`
  pull that plumbing out once; every call site above (plus
  `stylePhotoDialog`'s TUTUP button, previously an inline
  `.close()` with no fallback at all) now calls through them. This is
  a mechanics-only extraction -- each dialog's own custom content
  (cancel-reason textarea, login form, photo viewer, generic
  alert/confirm text) and all other behaviour are byte-for-byte
  unchanged; only the open/close DOM calls moved from 8 duplicated
  copies to 2 shared functions callers use.
- Verified: `node --check` on both script blocks, `npm test` (23/23
  fixtures + 20,000/20,000 differential + sql-consistency clean -- no
  domain/schema logic touched). New Playwright harness (31/31 checks)
  using the real `initTablistArrowNav()`, `setCustomerMode()`,
  `renderOperationalDayTabs()`, `openAppDialog()`/`closeAppDialog()`
  bodies extracted verbatim from `index.html`: confirms both tablists'
  initial ARIA state, that click still activates a tab (existing
  `onclick`, unchanged) and updates `aria-selected`/roving `tabindex`/
  panel `display` correctly, that arrow keys move focus and roving
  `tabindex` circularly (including wrap-around) and via Home/End
  *without* changing which tab is selected (the manual-activation
  requirement), that the weekly-hours panel's `aria-labelledby`
  follows the actual selected day after a real `selectOperationalDay()`
  click, and that `openAppDialog()`/`closeAppDialog()` against a real
  `<dialog>` element actually open/close it (`dialog.open` true then
  false) via the genuine `showModal()`/`close()` path. No SQL/migration
  changes.
- All 5 findings from the `/frontend-component-build` audit are now
