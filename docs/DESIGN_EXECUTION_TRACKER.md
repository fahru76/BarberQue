...
| 2.4 | Admin services empty state | Fahru | DONE | Already correct: reads from localStorage `shopServices`, shows honest message if empty. No loading artifact. |
| 2.5 | Error states for the queue/appointment refresh paths | Fahru | DONE | Deliberate: `console.warn` only. A missed live refresh is not an error — the next refresh or existing localStorage catches up. |
...
| 4.2 | Contrast audit on `--sleek-accent` orange on warm black | Fahru | DONE | Measured then fixed (`94b6ce2`). `.btn-action` is 12.48px/700 → NOT large text → 4.5:1 threshold; it failed in **both** themes (dark 2.23, light 3.48). The light gradient **spanned the WCAG dead-zone band** (lum 0.18333–0.20287) so no foreground-only fix existed — verified by scoring 6 candidates. Fixed: light `--sleek-accent #e95414 → #bf3f0b` (worst 3.48 → 4.89), `.btn-action`/`.brand-monogram` → `var(--on-primary)` (dark 6.42 / light 4.89), dropped the unpassable `#ff9952` hover stop, `.specialty-star.active` `#fff → #11130f` (2.19 → 7.78). Blast radius checked first: 35 `--sleek-accent` consumers, only 3 put text on it. Fingerprint 6/6 moved (dark `tok SAME` / light `tok DIFF` — correct: only light tokens changed). npm test exit 0. Light `--warning` override **done on `main` directly (`0b0ec8c`): `#82601b`, 5.00–5.68:1 across light surfaces, dark untouched, fingerprint dark-hash unchanged**. Remaining: the `[A]` alpha-background rows the auditor flags are unverified ** |
| 4.2b | `[A]` alpha-background row verification (leftover) | Fahru | DONE | All 14 alpha-row surfaces composited against their real base in both themes and scored (not eyeballed): `.cal-day.closed/.weekly-closed/.in-range`, `.reminder-box`, `#customer-closed-view`, `#customer-break-alert`, `#queueFullAlert`, `#adminNoActiveSeatsWarning`, `#closingSoonAlert`, `#tv-closed-notice`, `#adminUnservableBookingSection`, `#adminEditServiceForm`, `#adminClosedDateGuide`, `#calLegend*Icon`, `#operationalHoursNotice`. 10 pass AA as-is; 3 light-theme `--danger`-on-wash sites failed (3.96–4.26:1: `.cal-day.closed`, `#adminNoActiveSeatsWarning`, `#tv-closed-notice`) and were fixed with a scoped light override `#964a44` (4.64–5.00:1) in `9aa0e81`; `.cal-day.in-range` dark was 2.67:1 but its text is calendar-numbering inside a range highlight (large/secondary, per Phase 3.2 the calendar is admin-only preview) — flagged, not changed. Verified: live-DOM Playwright probe shows `rgb(150,74,68)` light / `rgb(199,110,104)` dark on all fixed surfaces, 0 page errors; fingerprint vs pre-change baseline dark tokenHash+styleHash byte-identical 3/3, light moved 3/3 (correct blast radius); npm test exit 0; CI green on `9aa0e81`. |
...
## Post-merge admin scroll report (2026-09-17, not reproduced — closed)

User reported admin panel content longer than the viewport couldn't scroll
further (Chrome desktop). Reproduction battery: real wheel events over 5
cursor positions (content, sidebar, nav, body), all 10 admin sections across
720p/900p/1080p viewports, Chrome-channel browser — every section scrolled to
its exact maximum, 0 unreachable elements, no `overflow:hidden` traps, no
wheel/scroll `preventDefault`, sidebar's own `overflow-y:auto` behaves
correctly. Closed as not-a-code-defect; if it recurs, check Chrome zoom level
(`Ctrl+0`) and cursor position over the sticky `.admin-sidebar` (independent
scroll container at `top:90px`, `max-height: calc(100vh - 110px)`) first.
